    ...
    def perform_command(self):
        ...
        if download_from_youtube:
            ...
            if audio_extension.lower() not in AudioFile.FILE_EXTENSIONS:
                self.print_warning(
                    u"Your audio file path has extension '%s', which is uncommon for an audio file." % audio_extension)
        ...
        if print_faster_rate:
            ...
            if max_rate is not None:
                faster = [l for l in task.sync_map_leaves(SyncMapFragment.REGULAR) if
                          l.rate >= max_rate + Decimal("0.001")]
                if len(faster) > 0:
...
def main(txt_path, audio_path, srt_path):
    """
    Execute program.
    """
    ExecuteTaskCLI().run(arguments=['', audio_path,
                                    txt_path,
                                    'task_language=zho|is_text_type=plain|os_task_file_format=srt',
                                    srt_path])


if __name__ == '__main__':
    the_txt_path = '/Users/chenjunming/Downloads/aeneas-1.7.3.0/x4_yeting.mp3'
    the_audio_path = '/Users/chenjunming/Downloads/aeneas-1.7.3.0/input.txt'
    the_srt_path = '/Users/chenjunming/Downloads/aeneas-1.7.3.0/output_txt.srt'

    main(txt_path=the_txt_path, audio_path=the_audio_path, srt_path=the_srt_path)
